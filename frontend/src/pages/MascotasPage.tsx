          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {filtered.map((m) => {
                ...
                const ultimoEvento = m.historial && m.historial.length > 0
                  ? m.historial[m.historial.length - 1]
                  : null;
                const historialReciente = m.historial && m.historial.length > 0
                  ? m.historial.slice(-3).reverse()
                  : [];
                return (
                  <div key={m.id} className="corp-card overflow-hidden hover:shadow-md transition-shadow">
                    ...
                    <div className="p-4">
                      ...
                      <div className="mt-3 flex flex-wrap items-center gap-4 text-xs font-semibold text-slate-500">
                        ...
                        <span className="inline-flex items-center gap-1">
                          <LuActivity className="w-3.5 h-3.5" />
                          {m.horas_uso ?? 0} h de uso
                        </span>
                      </div>
                      {ultimoEvento && (
                        <div className="mt-2 text-xs text-slate-500 font-medium">
                          Ultimo evento: {ultimoEvento.estado} - {ultimoEvento.fecha}
                        </div>
                      )}
                        ...
                        <div className="mt-3 pt-3 border-t border-slate-100">
                          ...
                          <ul className="mt-2 space-y-1 text-xs text-slate-600">
                            {historialReciente.map((evento, idx) => (
                              <li key={`${evento.fecha}-${idx}`} className="flex items-start gap-2">
